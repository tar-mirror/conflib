/*
    conflibtest.c - main() to test conflib
    Copyright (C) 1993,1994,1995,1996,1997  Uwe Ohse, 1991, 1993 Wolfram Roesler
...
cl_var_t Tab[] =
{
	{ "string",    NULL,       CL_STRING,  &string     ,0          ,NULL },
	...
	{ "array",     NULL,       CL_ARRAY,   array       ,0          ,NULL },
	...
	{ "dummy",     NULL,       CL_DUMMY,   NULL        ,0          ,NULL },
	{ "space-list"," ",        CL_LIST,    &sliste     ,0          ,NULL },
	{ "multi-list",": ,\t",    CL_LIST,    &mliste     ,0          ,NULL },
	{ "char",	   NULL,       CL_CHAR,    &character  ,0          ,NULL },
	{ "tmpdir",    NULL,       CL_DIRNAME, &tmpdir     ,0          ,NULL },
/* check CL_PREENV/POSTENV */
	{ "tmpdirP",   "TMP",      CL_DIRNAME, &tmpdirP    ,CL_POSTENV ,NULL },
	{ "Ptmpdir",   "TMP",      CL_DIRNAME, &Ptmpdir    ,CL_PREENV  ,NULL },
/* check CL_RANGE_REGEX" */
	{ "twodigits", NULL,       CL_NUMSHORT,&twodigits  ,CL_RANGE_REGEX, "[0-9][0-9]"  },
/* check CL_RANGE_WILDCARD */
	{ "checkstr",  NULL,       CL_STRING,  &checkstr   ,CL_RANGE_WILDCARD, "[A-Z]*"  },
/* check CL_WARNING */
	{ "aliaswarn", "aliasziel",CL_ALIAS,   NULL        ,CL_WARNING, NULL },
	{ NULL,NULL,0,NULL,0 }
};
...
int main(int argc, char **argv)
{
	...
	cl_var_t *v;
	cl_ulist_t *unknowns=NULL;
	cl_file_t *handle=NULL;
	const char *stanza="*";
	int u_flag=0;
	const char *mode="r";

	opterr = 0;
	while ((c = getopt (argc, argv, "di:c:s:v:V")) != -1) {
		switch (c)
		{
		case 'i':
			{
				char *p=tip_ainterpret(mytiptab,optarg);
				if (!p) {
					printf("conflibtest: can't interpret '%s'\n",optarg);
				} else  {
					printf("%s\n",p);
				}
			}
		...
		case 'c':
			...
			handle=cl_openconf(fname,mode);
			if (!handle) {
				printf("cannot open %s: %s\n",
				fname,strerror(errno));
				exit(1);
			}
			handle->report_unknown=0;
			break;
		case 'd': /* dump */
		...
		case 's':
			...
			if (*optarg == '\0')
				stanza=NULL;
			cl_clearconf(Tab);
			if (0!=cl_getstanza(handle,Tab,stanza,0,
				u_flag ? &unknowns : NULL))
			{
				printf("cannot read stanza %s\n",optarg);
			}
			break;
		case 'v':
			v=cl_find_confvar(Tab,optarg);
			if (!v) {
